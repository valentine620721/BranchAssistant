...
def generate_table_inform(table_inform, msg):
    system = f'''
        ...
        ```
        1. Tables: Table Name (table1 or table1 JOIN tables2 on table1.XXX=table2.XXX if necessary)
        2. Fields to be used: Fields Name | Metadata | Datatype, ...
        ```<END>
...
def query_by_SQL():
    with open(SummaryFunc.database+"tables.txt", 'r') as file:
        lines = file.read().split('\n')

    table_inform = lines[0]+ '\n'

    for table_name in SummaryFunc.Action_Output.split(','):
...
def reload():
    ...
    class SearchOracleInput(BaseModel):
        action_input: str = Field(description=f'''
            List all Table Name to be queried and JOIN, ex: table1, table2, ...
