...
@app.route("/get_check_and_query", methods=['POST', 'OPTIONS'])
@cross_origin()
def get_check_and_query():
    ...
    try:
        Ai_check = ""
        Ai_check = Agent_Executor.invoke({"input": user_input})['output'].replace("A.", "")
        ...
        return make_response(jsonify({'check': Ai_check, 'query': result}), 200, header)

    except Exception as e:
        result = SummaryFunc.sqlCommand #無法生成
        print(f'\nThe error message is here: {e}')
